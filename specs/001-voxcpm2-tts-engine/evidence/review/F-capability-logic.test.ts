// Review F1-F3 additions (downloadDecision, PRE_CAPABILITY_ENGINES, shouldFallBackFromEngine,
// engineUnavailableFallback, engineNotice's translate argument) are at the end of this file.
// T049 additions (voiceDesignEngineOptions, pickVoiceDesignEngine, designPromptError) are at the end of this file.
...
import { isProfileCompatibleWithEngine } from 'D:/Projects/Personal/voicebox/app/src/components/Generation/EngineModelSelector';
import {
  ENGINE_LANGUAGES,
...
// A stand-in for i18next's t: it shows the key and the interpolated name.
const fakeT = (key: string, params?: Record<string, unknown>) => `${key}|${String(params?.name)}`;
...
describe('T031 engineNotice', () => {
  test('an unavailable engine shows its reason, and stays listed as not selectable', () => {
    ...
    expect(engineNotice(cap, fakeT)).toEqual({ kind: 'reason', text: REASON });
  ...
  test('an unavailable engine without a reason still explains itself, using its display name', () => {
    const notice = engineNotice(capability({ available: false, reason: null }), fakeT);
    expect(notice?.kind).toBe('reason');
    expect(notice?.text).toContain('Engine Under Test');
  });

  test('an available engine shows its warning', () => {
    expect(engineNotice(capability({ warning: WARNING }), fakeT)).toEqual({ kind: 'warning', text: WARNING });
  });
  ...
  test('a warning never blocks: the engine stays selectable (C1Q4)', () => {
    ...
    expect(engineNotice(cap, fakeT)?.kind).toBe('warning');
  });

  test('nothing to show for a clean engine or an unreported one', () => {
    expect(engineNotice(capability(), fakeT)).toBe(null);
    expect(engineNotice(undefined, fakeT)).toBe(null);
  });
});
...
// ── T049: the "Describe a voice" profile source (FR-015a), decided by capability data only ──
describe('T049 voiceDesignEngineOptions', () => {
  const caps = (engines: Record<string, unknown>[]) =>
    ({ engines: engines.map((e) => capability(e)) }) as never;

  test('lists only engines declaring supports_voice_design, labelled with display_name', () => {
    const options = voiceDesignEngineOptions(
      caps([
        { engine: 'a', display_name: 'Engine A', supports_voice_design: true },
        { engine: 'b', display_name: 'Engine B', supports_voice_design: false },
        { engine: 'c', display_name: 'Engine C', supports_voice_design: true, available: false },
      ]),
    );
    expect(options).toEqual([
      { value: 'a', label: 'Engine A', available: true },
      { value: 'c', label: 'Engine C', available: false },
    ]);
  });

  test('nothing when no engine declares it, or capabilities are not loaded', () => {
    expect(voiceDesignEngineOptions(caps([{ supports_voice_design: false }]))).toEqual([]);
    expect(voiceDesignEngineOptions(undefined)).toEqual([]);
  });

  test('no engine name decides the result', () => {
    const options = voiceDesignEngineOptions(
      caps([
        { engine: 'kokoro', display_name: 'Kokoro', supports_voice_design: true },
        { engine: 'voxcpm', display_name: 'VoxCPM2', supports_voice_design: false },
      ]),
    );
    expect(options.map((o) => o.value)).toEqual(['kokoro']);
  });
});

describe('T049 pickVoiceDesignEngine', () => {
  const options = [
    { value: 'x', label: 'X', available: false },
    { value: 'y', label: 'Y', available: true },
    { value: 'z', label: 'Z', available: true },
  ];

  test('keeps the current choice when it is a voice-design engine', () => {
    expect(pickVoiceDesignEngine(options, 'z')).toBe('z');
    expect(pickVoiceDesignEngine(options, 'x')).toBe('x');
  });

  test('otherwise the first available voice-design engine', () => {
    expect(pickVoiceDesignEngine(options, 'not-listed')).toBe('y');
    expect(pickVoiceDesignEngine(options, '')).toBe('y');
    expect(pickVoiceDesignEngine(options)).toBe('y');
  });

  test('the first declared one when none is available, and nothing when none is declared', () => {
    expect(pickVoiceDesignEngine([{ value: 'x', label: 'X', available: false }])).toBe('x');
    expect(pickVoiceDesignEngine([])).toBe('');
  });
});

describe('T049 designPromptError', () => {
  test('required: blank or missing text is an error', () => {
    expect(designPromptError('')).toBe('required');
    expect(designPromptError('   \n')).toBe('required');
    expect(designPromptError(undefined)).toBe('required');
  });

  test('at most 2000 characters, matching the backend (models.py design_prompt)', () => {
    expect(MAX_DESIGN_PROMPT_CHARS).toBe(2000);
    expect(designPromptError('a'.repeat(2000))).toBe(null);
    expect(designPromptError('a'.repeat(2001))).toBe('tooLong');
  });

  test('any language is accepted (C1Q9)', () => {
    expect(designPromptError('صوت امرأة هادئ ودافئ')).toBe(null);
  });
});

describe('T049 isProfileCompatibleWithEngine, designed profiles', () => {
  const designed = { voice_type: 'designed', design_prompt: 'A calm voice' } as never;

  test('a designed profile fits only engines whose capability declares voice design', () => {
    expect(isProfileCompatibleWithEngine(designed, 'any', capability({ supports_voice_design: true }))).toBe(true);
    expect(isProfileCompatibleWithEngine(designed, 'any', capability({ supports_voice_design: false }))).toBe(false);
    expect(isProfileCompatibleWithEngine(designed, 'qwen', undefined)).toBe(false);
  });

  test('no engine name decides the result', () => {
    expect(
      isProfileCompatibleWithEngine(designed, 'kokoro', capability({ engine: 'kokoro', supports_voice_design: true })),
    ).toBe(true);
    expect(
      isProfileCompatibleWithEngine(designed, 'voxcpm', capability({ engine: 'voxcpm', supports_voice_design: false })),
    ).toBe(false);
  });

  test('cloned and preset profiles are unchanged', () => {
    const cloned = { voice_type: 'cloned' } as never;
    const preset = { voice_type: 'preset', preset_engine: 'p' } as never;
    expect(isProfileCompatibleWithEngine(cloned, 'qwen', undefined)).toBe(true);
    expect(isProfileCompatibleWithEngine(cloned, 'other', capability({ supports_cloning: false }))).toBe(false);
    expect(isProfileCompatibleWithEngine(preset, 'p', undefined)).toBe(true);
    expect(isProfileCompatibleWithEngine(preset, 'q', undefined)).toBe(false);
  });
});

// ── Review F2: the download confirmation fails closed (FR-018) ──
// A capability-channel engine is any engine outside the pre-capability set; the tests use a
// made-up id so no real engine name decides anything.
const LEGACY = new Set(['legacy-engine']);
const NEW_ENGINE = 'engine-under-test';

describe('F2 PRE_CAPABILITY_ENGINES', () => {
  test('is the set of engines with an app-side language list (the scope boundary)', () => {
    expect([...PRE_CAPABILITY_ENGINES].sort()).toEqual(Object.keys(ENGINE_LANGUAGES).sort());
  });
});

describe('F2 downloadDecision', () => {
  test('capability requires confirmation and the model is not downloaded: confirm', () => {
    expect(downloadDecision(NEW_ENGINE, capability(), { downloaded: false }, LEGACY)).toBe('confirm');
  });

  test('capability requires confirmation but the model state is unknown: still confirm', () => {
    expect(downloadDecision(NEW_ENGINE, capability(), undefined, LEGACY)).toBe('confirm');
  });

  test('model already downloaded or downloading: proceed, with or without a capability', () => {
    expect(downloadDecision(NEW_ENGINE, capability(), { downloaded: true }, LEGACY)).toBe('proceed');
    expect(
      downloadDecision(NEW_ENGINE, capability(), { downloaded: false, downloading: true }, LEGACY),
    ).toBe('proceed');
    expect(downloadDecision(NEW_ENGINE, undefined, { downloaded: true }, LEGACY)).toBe('proceed');
    expect(
      downloadDecision(NEW_ENGINE, undefined, { downloaded: false, downloading: true }, LEGACY),
    ).toBe('proceed');
  });

  test('capability does not require confirmation: proceed', () => {
    expect(
      downloadDecision(
        NEW_ENGINE,
        capability({ requires_download_confirmation: false }),
        { downloaded: false },
        LEGACY,
      ),
    ).toBe('proceed');
  ...
  test('no capability for a capability-channel engine that may download: refuse', () => {
    expect(downloadDecision(NEW_ENGINE, undefined, { downloaded: false }, LEGACY)).toBe('refuse');
    expect(downloadDecision(NEW_ENGINE, undefined, undefined, LEGACY)).toBe('refuse');
  });

  test('no capability for a pre-capability engine, or a model with no engine: proceed as today', () => {
    expect(downloadDecision('legacy-engine', undefined, { downloaded: false }, LEGACY)).toBe('proceed');
    expect(downloadDecision(undefined, undefined, { downloaded: false }, LEGACY)).toBe('proceed');
  });

  test('a pre-capability engine that does report a capability follows it', () => {
    expect(downloadDecision('legacy-engine', capability(), { downloaded: false }, LEGACY)).toBe(
      'confirm',
    );
  });
});

// ── Review F1: no engine fallback before capabilities load, unless the engine is app-side ──
describe('F1 shouldFallBackFromEngine', () => {
  const base = {
    engine: NEW_ENGINE,
    currentEngineAvailable: false,
    optionCount: 3,
    capabilitiesLoaded: true,
    appSideEngines: LEGACY,
  };

  test('falls back when the engine is not offered and capabilities have loaded', () => {
    expect(shouldFallBackFromEngine(base)).toBe(true);
  });

  test('cold start: a capability-channel engine is kept until capabilities load', () => {
    expect(shouldFallBackFromEngine({ ...base, capabilitiesLoaded: false })).toBe(false);
  });

  test('cold start: an app-side engine is decided without capabilities', () => {
    expect(
      shouldFallBackFromEngine({ ...base, engine: 'legacy-engine', capabilitiesLoaded: false }),
    ).toBe(true);
  });

  test('never when the engine is offered, or when there is nothing to fall back to', () => {
    expect(shouldFallBackFromEngine({ ...base, currentEngineAvailable: true })).toBe(false);
    expect(shouldFallBackFromEngine({ ...base, optionCount: 0 })).toBe(false);
  });
});

// ── Review F3: the fallback reason is a translation key with the display name ──
describe('F3 engineUnavailableFallback', () => {
  test('returns the i18n key and the display name as its parameter', () => {
    expect(engineUnavailableFallback(capability({ available: false, reason: null }))).toEqual({
      key: 'engines.cannotRunHere',
      params: { name: 'Engine Under Test' },
    });
  });

  test('engineNotice translates the fallback through the given t, and keeps a backend reason as is', () => {
    expect(engineNotice(capability({ available: false, reason: null }), fakeT)).toEqual({
      kind: 'reason',
      text: 'engines.cannotRunHere|Engine Under Test',
    });
    expect(engineNotice(capability({ available: false, reason: REASON }), fakeT)?.text).toBe(REASON);
  });
});
