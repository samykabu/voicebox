...
- [x] Pydantic response models defined in `backend/models.py`. Evidence: `EngineAdvancedSettingResponse` (:620), `EngineCapabilityResponse` (:630) and `EngineCapabilitiesResponse` (:659).
- [x] TypeScript client regenerated with `bun run generate:api`. The generated client (`app/src/lib/api/models/`, `services/` and `core/`) **must not** be hand-edited. `app/src/lib/api/types.ts` is a separate hand-written file and is edited normally. Evidence: `getEngineCapabilitiesModelsEnginesGet()` in `app/src/lib/api/services/DefaultService.ts:2278`, calling `/models/engines`; `app/src/lib/api/models/EngineCapabilityResponse.ts` carries the "generated using openapi-typescript-codegen -- do not edit" header.
- [x] `GenerationRequest` gains two optional fields, both defaulting to `None` so every existing caller and agent integration is unaffected: `voice_description: str | None` (max 500 characters, matching `instruct`) and `advanced_settings: dict[str, float] | None`. A value outside a declared setting's bounds, or a name the engine does not declare, is rejected with a 422. Evidence: `backend/models.py:108-138`, the two fields and the `_check_advanced_settings` validator. When `engine` is null, the model skips this check; `/generate` and `/generate/stream` run it with the same message after resolving the engine (request engine, else the profile's default) and return 422 with a string `detail`.
- [x] `backend/README.md` updated to document the endpoint. Evidence: the "Engine capabilities" section, `backend/README.md:110-161`.
- [x] New fields are additive and optional-by-default so existing agent integrations keep working untouched. Evidence: the endpoint is new, both request fields default to `None`, and the five `ModelConfig` capability fields default to today's behaviour (see T009 in tasks.md). No existing field or route was removed. The only changed lines in `backend/models.py` widen the existing `engine` and `language` patterns to accept new values (`git diff main...HEAD -- backend/models.py`), so every value accepted before is still accepted.
