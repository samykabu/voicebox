...
- [x] T037 [REVIEW] [HUMAN-REVIEW] [US4] Consent and disclosure review of the voice design path (Principle II)
...
- [x] T050 [US4] Show a responsible-use acknowledgement in the profile dialog for every source
...
- [x] T051 [TDD] Tag every saved audio file as AI-generated
...
- [x] T054 [TDD] Tag generated audio that bypasses save_audio
  - Test first, then add a small helper that writes the same RIFF INFO disclosure into in-memory WAV bytes, and use it in `backend/services/tts.py` (about line 32; /generate/stream, and `generate_audio_sync`, which has no live route caller) and `backend/routes/effects.py` (about line 53; effect preview). Audio samples must be unchanged apart from the metadata. Read the tag back in tests. JUnit counts from XML. `/speak` and the MCP speak tool are not in this task: they call `generate_speech` (`backend/routes/speak.py:72-83`, `backend/mcp_server/tools.py:250-263`), so their audio is saved through `save_audio`, which tags generated audio by default. The non-persisted surfaces tagged through `wav_bytes` are `/generate/stream` and the effects preview. **Covers**: FR-026.
...
**Review fixes (2026-09-23, 20c4224, 55ef593, ec1dbc3).** VoxCPM2 declares `retries_runaway=False`; load and inference are locked; designed long text keeps one speaker by cloning chunk 0; a one-off `voice_description` is stored in a new nullable `generations.voice_description` column and replayed on retry and regenerate; a null request engine is validated after resolution. App: capability loading gates the engine fallback, the download confirmation fails closed, new strings go through i18n, and the profile draft keeps the description. Evidence: evidence/review/.
...
- *Out of memory on a processor-only machine.* There, running out of system memory can make the operating system stop the backend process, instead of the load raising an error. The probe measured a CPU peak RSS of about 11.0 GB (Windows peak working set 11.2–11.5 GB), mostly from loading ([evidence/probe.md](./evidence/probe.md), row 8 and the summary at line 127). No warning guards this today: `detect_accelerator_and_memory` in `backend/backends/base.py` reads memory only for CUDA, so a processor-only machine gets no memory warning.

**H1 behaviour change after the analyze revalidation (2026-09-23).** FR-023 asks for processor fallback. Commit 699f362 changed `resolve_engine_availability` in `backend/backends/base.py`. An engine that declares `cpu` but detects an accelerator it does not declare, such as Intel XPU or DirectML, is now available and runs on the processor, with a warning instead of a reason. Only an engine that declares no processor path is refused, and no registered engine does that today. Tests: `backend/tests/test_cpu_fallback.py`. The T044 Step 5 DirectML/XPU evidence, which showed VoxCPM2 refused, is superseded: see [evidence/revalidation/T044-directml-xpu-superseded.md](./evidence/revalidation/T044-directml-xpu-superseded.md). T008's detail now states the new rule.
