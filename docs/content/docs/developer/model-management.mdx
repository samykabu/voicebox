...
- **Hardware.** CUDA, Apple Silicon (MPS) and CPU (`accelerators=("cuda", "mps", "cpu")`). The probe tested CUDA and CPU; MPS has an upstream code path but has not been tested. Because it declares `cpu`, it stays available on any other accelerator, such as Intel XPU or DirectML. There it runs on the processor, and `warning` says so: "VoxCPM2 (Multilingual, Voice Design) doesn't support Intel XPU here, so it will run on the processor, which is slower."
...
3. The dialog shows the size from `size_mb`, the licence from `license_id` and `commercial_use`, and any `warning` (memory or processor fallback). For VoxCPM2 it reads "This downloads about 4.8 GB before it can be used. Licence: Apache-2.0, commercial use allowed." (the dialog divides by 1024).
...
`reason` is set if and only if `available` is false. `warning` is set in two cases. A CUDA device has less memory than the engine's `min_memory_mb`. Or the engine runs on the processor because it does not support the detected accelerator (VoxCPM2 on Intel XPU or DirectML). The existing engines declare no accelerators, so they are always available and never warn. See [TTS Engines](/developer/tts-engines#the-capability-channel) for every field.
...
| 400 with "can't run on this machine" | The engine declares neither this machine's accelerator nor `cpu`. No registered engine does this today | Use another engine. The `reason` names what the engine needs |
| VoxCPM2 "will run on the processor" warning | Intel XPU or DirectML machine. VoxCPM2 supports only CUDA, MPS and CPU | Advisory only. Generation works, but more slowly |
