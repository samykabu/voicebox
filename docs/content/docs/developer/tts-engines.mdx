...
|-------|---------|-------------|
| `accelerators` | `()` (unconstrained) | The engine runs only on some accelerators, e.g. `("cuda", "mps", "cpu")`. On any other accelerator the engine is shown greyed out with a reason and generation is refused. |
...
- **Device.** `get_torch_device(allow_mps=True)`, passed to the vendor explicitly. XPU and DirectML are not allowed, because the vendor accepts only `cpu`, `mps` and `cuda[:N]`.
...
| `warning` | Advisory, e.g. too little memory. Never blocks |
...
`min_memory_mb` is not in the response. It only drives `warning`. Memory is read only for CUDA devices today, so the warning appears only there.

The app reads capabilities, not engine names. The rules live in `app/src/lib/hooks/engineCapabilityRules.ts`. There is no `if (engine === 'voxcpm')` capability branch in `app/`. An engine this machine cannot run is shown greyed out with its `reason`, never hidden. Every route that starts a generation (`/generate`, `/generate/stream`, retry and regenerate) refuses it with a 400 and the same reason, before any model load or download. The other engines declare no accelerators, so they are never refused this way.
...
- `DownloadConfirmDialog.tsx` asks before the first download. It shows the size, the licence and any memory warning. It opens from the generate box and from Model Management.
