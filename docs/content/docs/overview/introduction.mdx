...
- **Voice cloning** — 7 cloning engines covering up to 30 languages. Zero-shot
  cloning from a reference sample (see [Voice Cloning](/overview/voice-cloning))
- **Voice design** — describe a voice in words and VoxCPM2 creates it, with no
  recording needed (see [Creating Voice Profiles](/overview/creating-voice-profiles#workflow-c--designed-profiles))
...
Nine engines with different strengths, switchable per-generation:
...
| **F5-TTS / Habibi** | Cloned | Arabic | Arabic dialect checkpoints |
| **VoxCPM2** | Cloned or Designed | 30 | Apache-2.0, 48 kHz, voice design from a written description |

An engine your computer can't run is shown greyed out with the reason.
...
| TTS Engines | Qwen3-TTS, Qwen CustomVoice, LuxTTS, Chatterbox, Chatterbox Turbo, TADA, Kokoro, F5-TTS, VoxCPM2 |
