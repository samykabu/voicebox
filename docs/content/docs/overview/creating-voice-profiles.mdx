...
description: "How to create voice profiles: cloned, preset and designed from a description"
---

## Overview

A **voice profile** is a saved voice you can reuse across generations, stories, and the API. Voicebox profiles come in three flavors that map to three different ways of getting a voice. In the profile dialog they are the three sources: **Clone from audio**, **Built-in voice** and **Describe a voice**.
...
| **Designed**   | A written description of the voice                    | You want a new voice and have no recording to clone from |

All three types live in the same Profiles tab and behave the same way at generation time — pick the type that matches your goal and follow the workflow below.

<Callout type="info">
  Not sure which to use? Cloning gives you a *specific* voice but needs clean audio. Preset gives you *good* voices instantly but you don't get to choose who they sound like. Designed lets you describe the voice you want, but the result varies from one generation to the next more than a clone does.
</Callout>

<Callout type="warn">
  The profile dialog shows this line for every source: "Only create voices you have the right to use." It links to the [Responsible Use](https://github.com/samykabu/voicebox/blob/main/RESPONSIBLE_USE.md) guide. Read it before cloning anyone's voice.
</Callout>
...
<Steps>
  ...
  <Step title="Create Profile">
    **Profiles** → **+ New Profile** → choose a cloning engine (Qwen3-TTS, Chatterbox Multilingual, Chatterbox Turbo, LuxTTS, TADA, F5-TTS or VoxCPM2)
  </Step>
  <Step title="Upload or Record Sample">
    Drag in an audio file, or record directly with the in-app recorder. Enter the exact words spoken in the sample. VoxCPM2 cannot clone without this transcript
  </Step>
...
## Workflow C — Designed Profiles

Use this when you want a new voice and have nothing to clone from. You write a description and the engine creates a voice that matches it. Only engines that support voice design offer this source. Today that is **VoxCPM2**.

<Steps>
  <Step title="Choose the Source">
    **Profiles** → **+ New Profile** → **Describe a voice**. The option appears only when an engine supports voice design
  </Step>
  <Step title="Describe the Voice">
    Write how it should sound: age, gender, tone, pace, accent. For example, "A calm older woman with a low, warm voice". Any language works. Up to 2000 characters
  </Step>
  <Step title="Name and Save">
    No audio sample is needed. The description can't be changed later; create a new profile to try a different one
  </Step>
  <Step title="Generate">
    Pick the profile with VoxCPM2 and generate. Each generation uses the saved description
  </Step>
</Steps>

<Callout type="info">
  The same text and seed give the same audio. A different seed may give a noticeably different voice from the same description.
</Callout>
...
- **Export** profiles to share or backup. The file is a ZIP
- **Import** from colleagues or teammates
- **Cloned profiles** export with their audio samples and reference text
- **Preset profiles** export as engine + voice ID metadata only — the importer must have that engine's model installed
- **Designed profiles** export with their written description and no audio
- Every export keeps how the voice was made: its type, its description and its default engine. Import restores them, so a designed or preset profile does not come back as a clone
- Profiles exported by older versions still import, as cloned profiles
- The personality is not exported
