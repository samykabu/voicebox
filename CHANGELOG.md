...
- **New engine: VoxCPM2.** Apache-2.0, so commercial use is allowed. 48 kHz
  output and 30 languages, including Arabic. Runs on NVIDIA (CUDA) and the CPU.
  Apple Silicon is enabled too but has not been tested yet. The pronunciation
  dictionary applies to it as it does to the other engines.
- **Seven new languages.** Burmese, Indonesian, Khmer, Lao, Tagalog, Thai and
  Vietnamese. For now only VoxCPM2 offers them.
- **Asks before the download.** The model is about 5 GB (4961 MB). The first
  download shows its size and licence and waits for you to confirm. A warning
  appears when the graphics card looks short of memory; it does not stop you.
- **Clone voices with it.** Existing voice profiles work, including profiles
  with several samples. Each sample needs its transcript.
- **Design a voice from a description.** Create a profile with the new
...
- **The profile dialog asks you to create only voices you have the right to
  use**, for every kind of profile, and links to the Responsible Use guide.
- **Generated audio is labelled.** Every audio file Voicebox generates carries
  an "AI-generated by Voicebox" comment in its WAV metadata. Your own
  recordings, such as voice samples, are not labelled.
- **Profile export keeps how a voice was made.** Exported profiles carry their
  voice type, voice description and default engine, and import restores them.
  Designed and built-in voice profiles can be exported without audio samples.

### API changes

All of these are additive. Existing requests and files keep working.

- New `GET /models/engines`: each engine's capabilities, languages, licence,
  download size and whether this machine can run it.
- `POST /generate` and `/generate/stream` accept two new optional fields,
...
- Generation export manifests are now version 1.1 and carry `voice_description`, `engine` and `model_size`; import restores `engine` and `model_size` only after validating them against the TTS engine registry (an unknown engine falls back to the default engine, an invalid size to that engine's default size, each with a logged warning), and validates the imported `voice_description` like the API does: it is kept, stripped, only when it is a string of at most 500 characters and the restored engine supports voice design, otherwise it is stored as null with a logged warning and the import still succeeds. 1.0 archives import with no description and the default engine.
- Generated WAV audio now includes a RIFF `LIST/INFO` chunk (`ICMT` comment,
  ...
  Uploaded reference samples are stored without it.
- The TTS `language` field accepts `my`, `id`, `km`, `lo`, `tl`, `th` and `vi`.
