---
name: speckit-illustrate-generate
...
metadata:
  ...
  source: illustrate:commands/speckit.illustrate.generate.md
---
...
2. Resolve every relative `references/`, `assets/`, and `.specify/extensions/illustrate/scripts/` path against
   `.specify/extensions/illustrate/skill/`.
3. Resolve `.github/illustration-theme.yml` with the packaged `.specify/extensions/illustrate/scripts/illustration_theme.py`.
...
PNG, keep the HTML as the source of truth and use the bundled `.specify/extensions/illustrate/scripts/export_diagram.py` behavior
